...
# Pipeline simulation with plotting
def simulate():
    PC = 0
    cycle = 0
    ...
    # Generate pie chart
    instruction_cycles = cycle - memory_stalls - delayed_branches
    cycle_types = ['Instruction Execution', 'Memory Stalls', 'Branch Delay Slots']
    cycle_counts = [instruction_cycles, memory_stalls, delayed_branches]
    
    plt.figure(figsize=(8, 8))
    plt.pie(cycle_counts, labels=cycle_types, autopct='%1.1f%%', colors=['#66b3ff', '#ff9999', '#99ff99'])
    plt.title('Breakdown of Clock Cycles in MIPS Pipeline Simulation')
    plt.show()
